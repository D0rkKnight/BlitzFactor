// SRC: https://www.algorithms-and-technologies.com/bfs/javascript
...
/**
 * Implementation of Depth-First-Search (DFS) using adjacency matrix.
 * This returns nothing (yet), it is meant to be a template for whatever you want to do with it,
 * e.g. finding the shortest path in a unweighted graph.
 * This has a runtime of O(|V|^2) (|V| = number of Nodes), for a faster implementation see @see ../fast/DFS.java (using adjacency Lists)
 *
 * @param sdfsfdsf an adjacency-matrix-representation of the graph where (x,y) is true if the the there is an edge between nodes x and y.
 * @param start the node to start from.
 * @return Array array containing the shortest distances from the given start node to each other node
 */
const dfs = function (sdfsfdsf, start) {
    //A Stack to manage the nodes that have yet to be visited
    var stack = [];
    //Adding the node to start from
    stack.push(start);
    //A boolean array indicating whether we have already visited a node
    ...
    while (stack.length > 0) {
        ...
        //...for all neighboring nodes that haven't been visited yet....
        for (var i = 1; i < sdfsfdsf[node].length; i++) {
            if (sdfsfdsf[node][i] && !visited[i]) {
                // Do whatever you want to do with the node here.
                // Visit it, set the distance and add it to the stack
                visited[i] = true;
                distances[i] = distances[node] + 1;
                stack.push(i);
                console.log("Visiting node " + i + ", setting its distance to " + distances[i]);
            }
        }
    }
}